<div x-data="{ toast: null }"
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-4">
        <div class="relative max-w-sm">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search by name, phone or email..."
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="text-center px-4 py-3 font-semibold text-gray-600">Points</th>
                    <th class="text-right px-4 py-3 font-semibold text-gray-600">Balance</th>
                    <th class="text-center px-4 py-3 font-semibold text-gray-600">Sales</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-4 py-3 text-center">
                        <span class="bg-yellow-50 text-yellow-700 text-xs font-semibold px-2 py-0.5 rounded-full">{{ $customer->points }} pts</span>
                    </td>
                    <td class="px-4 py-3 text-right font-semibold text-gray-800">{{ setting('currency_symbol','৳') }}{{ number_format($customer->balance, 2) }}</td>
                    <td class="px-4 py-3 text-center">
                        <span class="bg-indigo-50 text-indigo-700 text-xs font-semibold px-2 py-0.5 rounded-full">{{ $customer->sales_count }}</span>
                    </td>
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            @if($confirmDelete === $customer->id)
                            <button wire:click="deleteCustomer" class="text-xs bg-red-600 text-white px-2 py-1 rounded hover:bg-red-700">Confirm</button>
                ...
                @empty
                <tr><td colspan="6" class="px-4 py-12 text-center text-gray-400">No customers found.</td></tr>
                @endforelse
            </tbody>
        </table>
